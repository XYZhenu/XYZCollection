...
- (instancetype)initWithCollectionViewLayout:(UICollectionViewLayout *)layout
{
    ...
    if (self) {
        ...
        [self custonBackButton];
        
    }
    
    return self;
}
-(void)custonBackButton{
    ...
    if (kind == UICollectionElementKindSectionFooter) {
        ...
        switch (self.filterType) {
            case QBImagePickerControllerFilterTypeNone:{
                footerView.textLabel.text = [NSString stringWithFormat:@"%ld 照片， %ld 视频",
                                             self.numberOfPhotos,
                                             self.numberOfVideos
                                             ];
                break;
            ...
            case QBImagePickerControllerFilterTypePhotos:{
                footerView.textLabel.text = [NSString stringWithFormat:@"%ld 照片",self.numberOfPhotos];
                break;
            }
                
            case QBImagePickerControllerFilterTypeVideos:{
                NSString *format = (self.numberOfVideos == 1) ? @"format_video" : @"format_videos";
                footerView.textLabel.text = [NSString stringWithFormat:@"%ld 视频",self.numberOfVideos];
                break;
            }
        }
